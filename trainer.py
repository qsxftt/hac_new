...
def load_exercises_from_db() -> List[Exercise]:
    ...
    try:
        ...
        if not exercises:
            logger.warning("⚠️ В БД нет упражнений")
            return []
        return exercises
...
def select_exercises_for_metrics(metrics: Dict[str, Any], limit: int = 7) -> List[Exercise]:
    """
    Подбор упражнений на основе метрик анализа
    
    Args:
        metrics: Словарь с метриками (avg_tempo, pauses_count, filler_words_count и т.д.)
    ...
    logger.info(f"Подбор упражнений для метрик: tempo={avg_tempo}, pauses={pauses_count}, fillers={filler_words_count}, reps={repetitions_count}")
    ...
    if exercises_added < limit:
        general_categories = ['diction', 'breathing', 'practice']
...
"""
        ...
        # Используем analyzer.analyze_speech, но передаем наш кастомный промпт
        # Для этого нужно немного хакнуть - создадим фейковый сценарий
        fake_scenario = {
            'type': 'custom',
            'text': prompt
        }
        
        ai_plan = analyzer.analyze_speech(
            transcript_text="",  # Пустой, т.к. всё уже в промпте
            analysis_results={},  # Пустой, т.к. всё уже в промпте
            scenario=fake_scenario
        )
        
        logger.info("✅ План от GigaChat получен")
        return ai_plan
...
def mark_exercise_completed(user_id: int, analysis_id: int, exercise_id: int, notes: str = None) -> bool:
    ...
    try:
        ...
        logger.info(f"✅ Упражнение {exercise_id} отмечено как выполненное для пользователя {user_id}")
        return True
